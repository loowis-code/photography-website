...
export default function PhotoForm({ mode = 'create', initialData=null, onSubmit }) {
    ...
    useEffect(() => {
        if (!initialData) return; // Only run if initialData is provided
        setForm({
            title: initialData.title ?? '',
            description: initialData.description ?? '',
            file: null,
	...
	useEffect(() => {
		const interval = setInterval(() => {
			...
			if (
				...
				const initialCoords = mapData.lat && mapData.lng ? [mapData.lat, mapData.lng] : [51.505, -0.09];
	...
	return (
		<form onSubmit={handleFormSubmit} className={styles.form}>
			...
			<div>
				<label className={styles.label} htmlFor="alt_text">Alt Text:</label>
				<textarea
					name="alt_text"
					id="alt_text"
					value={form.alt_text}
			...
			<div>
				<label className={styles.label} htmlFor="date_taken">Date Taken:</label>
				<input
					type="date"
					name="date_taken"
					id="date_taken"
			...
			<div>
				<label className={styles.label} htmlFor="camera">Camera:</label>
				<select
					id="camera"
					name="camera"
					...
					className={styles.input}
				>
					{(cameras.length ? cameras : [{ camera_id: null, brand: 'None', model: '' }]).map((camera) => (
						<option
							key={camera.camera_id || camera.brand + camera.model}
			...
			<div>
				<label className={styles.label} htmlFor="visible">Visible:</label>
				<input
					type="checkbox"
					name="visible"
					id="visible"
			...
			<div>
				<label className={styles.label} htmlFor="featured">Featured:</label>
				<input
					type="checkbox"
					name="featured"
					id="featured"
			...
			<div>
				<label className={styles.label} htmlFor="digital">Digital:</label>
				<input
					type="checkbox"
					name="digital"
					id="digital"
